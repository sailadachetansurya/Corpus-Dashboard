...
def apply_advanced_styling():
    """Apply advanced dark theme with glassmorphism effects"""
    st.markdown("""
    <style>
    ...
    /* Sidebar styling */
    .css-1d391kg {
        background: linear-gradient(180deg, rgba(15, 20, 25, 0.95), rgba(26, 29, 41, 0.95));
        backdrop-filter: blur(10px);
    }
...
def show_user_query_dashboard():
    """Show dashboard for querying any user's records"""
        ...
        if query_submitted and query_user_id:
            ...
            if records:
                ...
                if summary:
                    ...
                    create_advanced_overview_dashboard(summary)
                    create_export_options(records, f"user_{query_user_id[:8]}")
                else:
    ...
    if st.session_state.get("query_results") and st.session_state.get("last_queried_user"):
        st.markdown("---")
        st.markdown(f"### 📊 Previous Query Results for User: {st.session_state.last_queried_user[:8]}...")
        
        if st.button("🔄 Refresh Previous Query"):
            records = fetch_any_user_records(st.session_state.last_queried_user, st.session_state.token)
            if records:
                st.session_state.query_results = records
                st.rerun()
...
def show_comparison_dashboard():
    """Show comparison dashboard between users"""
    ...
    with col1:
        st.markdown("### 👤 User 1")
        user1_id = st.text_input("User 1 ID", key="compare_user1")
        
    with col2:
        st.markdown("### 👤 User 2")
        user2_id = st.text_input("User 2 ID", key="compare_user2")
            ...
            with st.spinner("Fetching comparison data..."):
                ...
                if records1 and records2:
                    create_comparison_dashboard(
                        records1, records2,
                        f"User {user1_id[:8]}...", f"User {user2_id[:8]}..."
                    )
                else:
                    if not records1:
                        st.error(f"No records found for User 1: {user1_id}")
                    if not records2:
                        st.error(f"No records found for User 2: {user2_id}")
        else:
            st.warning("Please enter both User IDs for comparison")
